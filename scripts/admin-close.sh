...
if grep -q "^ADMIN_CLOSED=true" "$ENV_FILE"; then
    echo "INFO: Admin is already closed. Nothing to do."
    exit 0
fi
...
if ! grep -q "^ADMIN_CLOSED=false" "$ENV_FILE"; then
    echo "ERROR: ADMIN_CLOSED=false not found in $ENV_FILE. Unexpected configuration." >&2
    exit 1
fi

sed -i "s/^ADMIN_CLOSED=false/ADMIN_CLOSED=true/g" "$ENV_FILE"
echo "✓ Admin closed"
