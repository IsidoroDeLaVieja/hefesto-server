...
class DeployApi implements DeployApiInterface
{
    ...
    private static function validate(array $api): void
    {
        ...
        foreach ($api['endpoints'] as $action => $endpoint) {
            [$verb, $path] = self::verbAndPathFromAction($action);
    ...
    private static function compile(
        ...
        foreach ($endpoints as $action => $endpoint) {
            ...
            $directives[$routeKey] = new SplDoublyLinkedList();
            $actions[] = [$verb, $path];

            self::addDirectivesToList($before, $directives[$routeKey], $release, $apiNamespace);
            self::addDirectivesToList($endpoint, $directives[$routeKey], $release, $apiNamespace);
            self::addDirectivesToList($after, $directives[$routeKey], $release, $apiNamespace);
        }
    ...
    private static function directiveRequest(
        ...
        return new DirectiveRequest(
            ...
            array_diff_key($config, ['directive' => true, 'groups' => true]),
            $groups,
        );
    }
    ...
    private static function template(
        ...
        return str_replace(
            ['#apiNamespace#', '#release#', '#actions#', '#directives#'],
            [$apiNamespace, $release, serialize($actions), serialize($directives)],
            $template,
        );
    }
